...
describe("Multipart Upload Endpoints", () => {
	...
	describe("PartUpload (POST /api/buckets/:bucket/multipart/upload)", () => {
		...
		beforeEach(async () => {
			if (!MY_TEST_BUCKET_1)
				throw new Error(
					"MY_TEST_BUCKET_1 not available for PartUpload beforeEach",
				);
		...
		it("should return 400 for a non-existent uploadId", async () => {
			...
			expect(errorText).toMatch(
				/The specified multipart upload does not exist/i,
			);
		});
	...
	describe("CompleteUpload (POST /api/buckets/:bucket/multipart/complete)", () => {
		...
		beforeEach(async () => {
			if (!MY_TEST_BUCKET_1)
				throw new Error(
					"MY_TEST_BUCKET_1 not available for CompleteUpload beforeEach",
				);
			...
			const partUploadResponse = await app.fetch(
				partUploadRequest,
				env,
				createExecutionContext(),
			);
			const partUploadBody = (await partUploadResponse.json()) as {
				etag: string;
				partNumber: number;
			};
		...
		it("should return 400 for completion with incorrect ETag", async () => {
			const completeBody = {
				...
				uploadId: uploadId,
				parts: [
					{ partNumber: part1Number, etag: "fake-etag-does-not-exist-123" },
				],
			};
			const request = createTestRequest(
				`/api/buckets/${BUCKET_NAME}/multipart/complete`,
